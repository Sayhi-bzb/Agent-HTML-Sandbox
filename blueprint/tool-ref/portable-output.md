...
Vite 属于 managed runtime、renderer adapter 或 template 边界，不属于 core engine。
...
- project-local 或 package-local Vite builder 不应继续存在，不应被描述为最终 engine boundary。
...
- 标记 managed runtime build 与 portable artifact 输出的边界。
