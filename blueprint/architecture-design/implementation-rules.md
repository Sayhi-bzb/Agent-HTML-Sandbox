...
新增、删除或修改 presentation profile 或 RenderConfig key / value 时，必须同步 schema、parse / sanitize 和 renderer adapter profile 注册。
...
presentation profile registry 必须是受控声明，不得从 renderer theme object、Tailwind config 或 shadcn props 自动外泄。
