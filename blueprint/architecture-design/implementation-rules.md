...
generated shadcn introspection 只能作为 schema 草稿、renderer registry 草稿和 drift check 输入。
...
shadcn-backed 组件接入必须优先走通用路径：introspection 抽取 exports / `data-slot` / safe props / dependencies，overlay 收束开放面，UiRegistry 注册，GenericRendererResolver 渲染。
