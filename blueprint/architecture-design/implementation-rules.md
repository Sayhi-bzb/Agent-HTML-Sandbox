...
parse、validate、sanitize、ComponentSchema、DocumentStyleConfigReference、RenderConfig、diagnostics 和公共类型应保持可被 CLI、adapter 和 tests 复用。
...
RenderConfig 默认应优先解析 document-level style config reference，不直接暴露完整 shadcn props 或平铺的视觉参数包。
...
新增、删除或修改 document style config reference、DocumentStyleConfig 或 RenderConfig key / value 时，必须同步 schema、parse / sanitize 和 renderer adapter 配置解析注册。
...
document style config catalog 生成或同步失败也不能静默通过。
...
最终 agent-facing ComponentSchema 和 DocumentStyleConfigReference 必须由显式声明收束，并经过 introspection 校验。
...
document style config catalog 必须是受控声明，不得从 renderer theme object、Tailwind config 或 shadcn props 自动外泄。
