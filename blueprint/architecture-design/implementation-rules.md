...
core engine 不得引入 Vite、React、shadcn/ui、Tailwind、DOM renderer、renderer adapter 或用户项目构建器依赖。
...
新增 shadcn component requirement 时，必须同步 renderer adapter、ComponentSchema 说明和 init / doctor 检查。
...
React、Tailwind、shadcn/ui 和 Vite 只能作为 renderer adapter、template、sandbox 或用户项目依赖。
...
## 19. CLI orchestrates user projects

CLI 可以创建或接入用户项目，并调用 shadcn CLI 初始化 template / preset。

CLI 必须明确区分 generated files、user files 和 package files。

CLI 不得把 shadcn component source 复制进 core，也不得把用户项目样式提升为 agent-facing 协议。
...
CLI 命令只能编排 schema、compose、parse / sanitize、renderer adapter、project integration 和 portable output。
