...
managed runtime 负责隔离承载 renderer adapter、Vite、React、Tailwind 和 shadcn/ui 实现。默认位置是用户级 `.ahtml` 目录，不是当前工作目录。
...
user installs ahtml globally
        ↓
user asks agent for an artifact
        ↓
agent writes standard agent-html document
        ↓
...
agent-html 使用固定结构的标准组件语法。组件风格、布局、间距和可组合结构由 managed runtime 预设，agent 只填写 props 和 slots。
...
shadcn components、theme、CSS variables 和 Tailwind 配置若被采用，只能属于 managed runtime。ahtml 可以编排 runtime bootstrap，但不把它们提升为 core engine 或 agent-facing 协议。
...
shadcn registry、组件源码和 managed runtime `components.json` 可用于自动生成 schema 草稿。
...
renderer adapter 负责把标准 agent-html 组件映射为 managed runtime 中的 UI 实现，并产出底层 HTML。
