...
```txt
ahtml config + engine + CLI + renderer adapters/templates
```

config 负责有限默认值、RenderConfig 值域、用户项目配置和 Vite/shadcn 集成元数据。

engine 负责 agent-html 文档边界、安全边界、ComponentSchema、diagnostics 和 `SanitizedAgentHtml`。

CLI 负责把 config 和 engine 接入用户项目。

renderer adapter 负责把 `SanitizedAgentHtml` 映射为具体 UI。React、shadcn/ui、Tailwind、Vite 和用户项目构建配置属于 adapter、template 或用户项目，不属于 core engine。
...
choose template / preset
        ↓
install user-local renderer adapter and shadcn components
        ↓
user project is ready for agent-html rendering
```
...
renderer adapter 将标准语法映射为用户项目中的 UI 实现，并产出可检查的底层 HTML。React、Tailwind class 和 shadcn/ui 细节不进入 agent-facing 主接口。
...
当前 package-local Vite builder 是兼容基线，不是最终 engine 边界。目标架构应把 Vite、React、shadcn/ui 和 Tailwind 迁移到 renderer adapter、template 或用户项目边界。
...
core 可以定义 ComponentSchema、RenderConfig、diagnostics 和 `SanitizedAgentHtml`，但不得依赖 Vite、React、shadcn/ui、Tailwind、renderer adapter 或用户项目构建配置。
...
shadcn components、theme、CSS variables 和 Tailwind 配置属于用户项目或 renderer adapter/template。ahtml package 可以编排安装，但不把它们提升为 core engine 或 agent-facing 协议。
...
## 4. schema automation drafts, overlay decides

shadcn registry、组件源码和用户项目 `components.json` 可用于自动生成 schema 草稿。
...
## 5. props schema / tokens are the agent-facing contract
...
## 6. parse / sanitize gates renderer adapter
...
## 7. renderer adapter maps standard syntax to UI

renderer adapter 负责把标准 agent-html 组件映射为用户项目中的 UI 实现，并产出底层 HTML。
...
## 8. artifact targets static sharing
...
## 9. CLI orchestrates the engine

CLI 是 agent-html engine 的主入口。

CLI 只编排 ComponentSchema、RenderConfig、parse / sanitize、renderer adapter、project integration 和 portable output。CLI 不定义平行协议，不绕过 sanitize，不暴露 shadcn props、Tailwind class、style、script 或外部资源逃逸口。

`ahtml init` 可以创建或接入用户项目，并安装 user-local renderer adapter 和 shadcn requirements。该能力不得把用户项目样式提升为 agent-facing 协议。
