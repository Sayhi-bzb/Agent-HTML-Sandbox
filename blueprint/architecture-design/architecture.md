...
agent-html 用 HTML artifact 替代冗长 Markdown agent 输出。agent-facing 层应让 agent 表达信息结构、关系、决策和反馈入口；最终产物应让人更容易阅读、比较、分享、归档，并把反馈带回 agent 工作流。
...
user installs ahtml globally
        ↓
user asks agent for a richer output than Markdown
        ↓
...
ui / slot capability schema
        ↓
agent reads prompt schema choices
        ↓
...
agent-html 使用固定结构的标准组件语法。组件风格、布局、间距和可组合结构由 managed runtime 预设，agent 只填写 props 和 slots。标准语法可以保留 `<card>`、`<tabs>` 等便捷形式，但 sanitize 后必须能归一到同一套 ui / slot capability model。

示例：

```html
<card title="标题"> 主体内容 </card>
```

renderer adapter 通过通用 registry / resolver 将标准语法映射为 managed runtime 中的 UI 实现，并产出可检查的底层 HTML。React、Tailwind class 和 shadcn/ui 细节不进入 agent-facing 主接口。
...
core 可以定义 ComponentSchema、RenderConfig、diagnostics 和 `SanitizedAgentHtml`，但不得依赖 Vite、React、shadcn/ui、Tailwind、renderer adapter 或 managed runtime 构建配置。
...
artifact build 仍然是产品主路径，但 build project 必须是 shadcn-template-derived managed runtime。ahtml 可以向其中注入 renderer app、SSR/build glue、sanitized document、rendererSpec、capability data 和 diagnostics，不得替换 shadcn template surface。
...
## 4. generic adaptation is the default

组件接入的默认工程路径是通用能力抽取、通用 ui / slot schema 和通用 renderer resolver。不得把“为这个组件写一个 adapter”作为默认方案。
...
shadcn registry / managed runtime components.json / component source
        ↓
extract exports / data-slot / safe props / dependencies
        ↓
ui component schema + ui slot schema
        ↓
generic renderer registry
        ↓
recursive resolver renders shadcn exports
```
...
## 5. schema automation drafts, overlay decides

shadcn registry、shadcn-native managed runtime、组件源码和 managed runtime `components.json` 可用于自动生成 schema 草稿和 renderer registry 草稿。
...
generated shadcn introspection 可抽取 exports、`data-slot`、`cva` variants、literal union props、registry metadata、dependencies、CSS/base surface identity 和 docs links。
...
自动化目标是减少 schema 和 renderer 维护成本，不是 100% 自动生成最终协议。实现可自动生成 70%-80% 的 schema / registry 草稿，但最终开放面必须由 overlay 白名单收束。

## 6. props schema / tokens are the agent-facing contract
...
## 8. renderer adapter resolves standard syntax to UI

renderer adapter 负责把标准 agent-html 组件通过通用 registry / resolver 映射为 managed runtime 中的 UI 实现，并产出底层 HTML。
