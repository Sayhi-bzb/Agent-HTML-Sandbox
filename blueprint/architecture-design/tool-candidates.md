...
- schema generation 可读取 managed runtime shadcn 材料，但输出必须经过 overlay 收束。
...
- Vite static build 可作为 managed runtime 输出。
- project-local 或 package-local Vite builder 不是兼容基线，也不是 core engine 边界。
- 交付阶段优先生成静态 artifact。
