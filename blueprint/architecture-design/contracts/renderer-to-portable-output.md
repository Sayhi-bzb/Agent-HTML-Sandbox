# renderer adapter to portable output Contract

本文规定 renderer adapter 到 React / HTML / portable output 的边界。

## Provider

renderer adapter 提供 React output 和 HTML output。
...
- 让 Vite、React、shadcn/ui 或 Tailwind 成为 core engine 依赖。
- 在输出阶段重新引入未检查脚本。
- 让交付物隐式依赖难以迁移的运行环境。
