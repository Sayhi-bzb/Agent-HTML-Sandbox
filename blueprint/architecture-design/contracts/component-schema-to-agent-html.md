...
agent 使用 schema 编写标准 agent-html，并选择受控视觉配置入口。全局样式和详细组件样式规则都留在独立配置层，不进入正文积木协议。
...
- render config header 或等价文档级配置入口只能选择 schema 暴露的 DocumentStyleConfigReference，并通过 `style-ref` 选择该完整样式方案引用。
...
- 全局主题 token 若存在，应通过独立配置层或外部引用进入，不应和正文语义节点混写；其命名与语义基线遵循 shadcn 官方 theming convention。
- 未经单独产品决策，不得再发明新的 component-level public config keys 或平行 visual syntax。
