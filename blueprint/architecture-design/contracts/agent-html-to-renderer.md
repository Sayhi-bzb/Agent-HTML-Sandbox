...
core parse / sanitize 层提供已检查的标准组件结构、语义 props、slot 结构和已批准的 profile 选择。
...
- parse / sanitize 必须校验组件名、props、slot 结构、children 边界和 profile 选择。
- 未知标签不得绕过安全边界执行。
...
- renderer adapter 把命名 profile 展开为 agent 必须理解的低层 shadcn 视觉参数。
