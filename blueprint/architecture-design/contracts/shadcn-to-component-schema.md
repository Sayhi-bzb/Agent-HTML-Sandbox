...
contract maintenance / renderer integration layer 消费 shadcn registry、组件源码、`components.json`、runtime capability facts 和显式语义声明，并产出 standardized component schema、presentation profile compatibility 和通用 renderer registry 输入。

## Rules

- shadcn/ui 是 UI 实现底座。
- 标准组件必须固定结构、固定语义用途和固定组合边界。
- GeneratedShadcnIntrospection 只表示内部 runtime capability facts，不表示公开协议。
- runtime capability facts 可自动抽取 exports、`data-slot`、dependencies、registry metadata、required files、required exports、docs links、CSS/base surface identity 和 blocked/internal prop candidates。
- `cva` variants、literal union props 或其他视觉实现细节最多作为审查材料和 drift check 输入，不应默认进入 agent-facing contract。
- ComponentSchemaOverlay 必须显式声明 expose、语义 props、slot 合法结构、children 边界、隐藏 props、profile 兼容性和安全 prop 映射边界。
- ComponentSchema 必须由显式语义声明收束，并经过 runtime capability facts 校验。
- ComponentSchema 只描述 agent-facing 信息。
- ComponentSchema 必须包含允许 props、允许 slots / children 和使用禁忌。
- PresentationProfile 必须由项目显式注册，并与 renderer 可支持的视觉策略保持一致。
...
- ComponentSchema 暴露默认要求 agent 决定 `variant`、`size`、`tone`、`surface`、`radius`、`spacing` 这类视觉实现参数。
