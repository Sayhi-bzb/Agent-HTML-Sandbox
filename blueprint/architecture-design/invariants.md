...
ahtml core 不依赖 Vite、React、shadcn/ui、Tailwind、renderer adapter 或用户项目构建配置。

这些依赖只能出现在 renderer adapter、template、sandbox 或用户项目中。
...
## 3. shadcn lives in the user project

shadcn components、theme、CSS variables 和 Tailwind 配置属于用户项目或 renderer adapter/template。

ahtml package 可以编排 shadcn 安装，但不把 shadcn 源码或样式作为 core 内置协议。
