...
Purpose: 表示一个命名的视觉策略档位，例如 `ops-compact`。
...
Note: PresentationProfile 可以在内部绑定 theme、density、card treatment、table treatment、badge treatment、emphasis 和 width 等受控 token，但这些 token 默认不直接暴露为 agent-facing 自由 props。

## 7. PresentationProfileRegistry

Ownership: presentation contract / CLI

Purpose: 表示项目公开给 agent 或用户选择的有限 profile 集合。
...
Note: PresentationProfileRegistry 是公开选择面，不是 renderer 内部 theme object、Tailwind config 或 shadcn variant 列表。
...
Note: RenderConfig 默认应只包含 profile id；只有明确受控且与 profile 绑定的少量 token 才能出现。它不是 CSS、style、className、Tailwind class、shadcn props、script 或外部资源入口。
...
Change rule: CliSchemaOutput 必须来自 ComponentSchema 和 PresentationProfileRegistry / RenderConfig，不得暴露 renderer props、Tailwind class、shadcn props 或源码结构。

## 19. CliConfigView

Ownership: CLI / presentation contract

Purpose: 表示 CLI 从 PresentationProfileRegistry 和 RenderConfig 派生的只读配置视图。
