...
<Callout title="Primary rule">
  ...
  names, props, children, and render config values. Runtime setup is automatic.
</Callout>
...
`ahtml status`, `ahtml doctor`, `ahtml build`, and `ahtml preview` initialize or repair the managed runtime automatically, including the internal shadcn-backed renderer. They do not write `src/`, `components.json`, `vite.config.ts`, or `agent-html.project.json` into the current repository.
