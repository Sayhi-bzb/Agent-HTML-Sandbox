...
The `ahtml` package owns the config, engine, CLI, and managed artifact runtime. Runtime files live under `~/.ahtml` or `%USERPROFILE%\.ahtml` by default, with `AHTML_HOME` as an override. Agents do not write arbitrary HTML, CSS, Tailwind classes, scripts, event handlers, React props, Radix props, or shadcn props.
...
## 1. Initialize the managed runtime [#initialize-the-managed-runtime]
...
`ahtml init` initializes or repairs the managed runtime. It does not write `src/`, `components.json`, `vite.config.ts`, or `agent-html.project.json` into the current repository.
