...
import { formatForbiddenPolicy } from "./agent-html-cli-contract.mjs"
...
async function loadSchemaModules(root) {
  ...
  let loader = schemaLoaders.get(resolvedRoot)

  if (!loader) {
    loader = loadSchemaModulesFromVite(resolvedRoot)
    schemaLoaders.set(resolvedRoot, loader)
  }

  return loader
}

async function loadSchemaModulesFromVite(root) {
  ...
  try {
    const [componentSchemaModule, renderConfigModule] = await Promise.all([
      server.ssrLoadModule("/src/agent-html/component-schema.ts"),
      server.ssrLoadModule("/src/agent-html/render-config.ts"),
    ])
