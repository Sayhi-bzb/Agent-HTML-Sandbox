...
| Component | Status | Why | Required contract/archetype | Next step |
| --- | --- | --- | --- | --- |
| `progress` | `direct-candidate` | 只读进度展示语义稳定，不依赖复杂 trigger 或 portal，适合 static artifact。 | 现有 `primitive`，第一版公开 props 应先收敛为 0-100 的 `value`。 | 先定义最小语义 contract，再补 runtime requirements、tests 和 artifact coverage。 |
