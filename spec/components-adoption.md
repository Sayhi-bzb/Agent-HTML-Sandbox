...
- `field-control`: supported, but still uses custom renderer wrappers and
  fallback markup rather than fully official shadcn composition patterns.
- `option-set`: supported, but `toggle-group`, `select`, and `combobox` still
  render through generic wrapper elements instead of the more official shadcn
  field/form composition used in current docs.
- `select`: supported, but the current runtime path flattens options directly
  into `SelectContent` and does not model the documented `SelectGroup`
  composition.
- `combobox`: supported, but the current runtime path only covers the minimal
  searchable-list flow; documented empty/grouped result-state composition is
  not represented yet.
- `fixtures`: runtime support is proven locally, but fixture snapshots and test
  registry responses can drift from upstream shadcn registry and docs.
