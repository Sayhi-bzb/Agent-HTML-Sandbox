...
当前没有仍停留在 `direct-candidate` 的组件。`progress`、`field/control` 两轮，以及
`option-set` 的首轮 `toggle-group` 都已经通过真实语义/runtime path 落地；下一批工作
应优先沿现有 `option-set` contract 扩到 `select` / `combobox`。
...
| Component | Status | Why | Required contract/archetype | Next step |
| --- | --- | --- | --- | --- |
| `select` | `needs-archetype` | 需要 option list、selected value、trigger/content 和 fallback 语义。 | 扩展现有 `option-set` archetype。 | 先统一 options contract，不直接暴露 Radix `Select` 结构。 |
| `combobox` | `needs-archetype` | 除 option list 外，还带搜索/过滤行为，复杂度高于 `select`。 | 扩展现有 `option-set` archetype。 | 在 `select` contract 成熟前，不单独接入。 |
...
- 单个只读显示型新增组件已经能低成本接入。
- `field/control` archetype 已能覆盖文本、布尔值和单组选项字段。
- `option-set` archetype 已能覆盖少量选项切换；下一步验证它能否继续覆盖受控选择器。
