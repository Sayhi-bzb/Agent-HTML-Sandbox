...
- Node ESM CLI exposed as `ahtml`, with `npm run agent --` kept as a compatibility fallback.
...
Expose the local CLI through the `ahtml` bin:
...
- Add an `ahtml` bin entry and keep the local `agent` npm script as a fallback.
...
- `ahtml schema --format prompt` prints agent-facing prompt text.
- `ahtml schema --format json` prints structured `CliSchemaOutput`.
- `ahtml schema --out <path>` writes the selected schema output.
...
- `ahtml config get` prints the effective `ArtifactConfig`.
- `ahtml config set <key> <value>` writes only known config keys and finite enum values.
...
- `schema`, `compose`, `build`, and `config` are available through `ahtml`.
